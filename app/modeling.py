...
'''
        ...
        if st.button("Perform Split", type="primary"):
            X = df.drop(target_col, axis=1)
            y = df[target_col]
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size/100, random_state=42)
...
'''
                ...
                train_button = st.button("Train Model & Evaluate", type="primary", use_container_width=True)
            ...
            with st.container(border=True):
                ...
                if train_button:
                    ...
                    if problem_type == "regression":
                        ...
                        fig = px.scatter(eval_df, x='Actual', y='Predicted', title="Actual vs. Predicted Values", opacity=0.6, color_discrete_sequence=['#3b82f6'])
